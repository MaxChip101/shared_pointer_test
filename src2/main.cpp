...
int main() {
    ...
    int share = get_share_point(SHM_NAME, O_RDONLY, 0666); // get an already created share point
    if (share == -1) return 1; // checks for errors
    char *pointer = create_shared_pointer(share, SHM_SIZE, PROT_READ, MAP_SHARED); // create a ointer that is shared
    if (pointer == nullptr) return 1; // checks for errors
    std::cout << read(pointer) << std::endl; // read the message
